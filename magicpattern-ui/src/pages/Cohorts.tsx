...
import {
  CreateCohortDrawer,
  CohortFormData,
  CreateCohortFooter } from
'../components/cohorts/CreateCohortDrawer';
import {
  CohortFilters,
  CohortStatus } from
'../components/cohorts/CohortFilters';
...
const cohortToFormData = (cohort: Cohort): Partial<CohortFormData> => {
  ...
  return {
    ...
    enrollmentDeadline: '' // In a real app, parse this from cohort.enrollmentDeadline
  };
};
export function CohortsPage({
  ...
  // Handle initial cohort selection from search
  useEffect(() => {
    if (initialCohortId) {
  ...
  const handleRestore = () => {
    ...
    {
      ...c,
      status: 'Upcoming' as const // Default restore state
    } :
    c
    )
    );
