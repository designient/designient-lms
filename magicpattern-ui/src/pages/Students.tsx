...
import {
  StudentFilters,
  StudentStatus } from
'../components/students/StudentFilters';
import { StudentsTable } from '../components/students/StudentsTable';
import { StudentDrawer } from '../components/students/StudentDrawer';
import {
  StudentForm,
  StudentFormData,
  StudentFormFooter } from
'../components/students/StudentForm';
...
},
{
  ...
  notes: [
  {
    ...
    authorName: 'Sarah Chen',
    authorRole: 'Mentor',
    content: 'Behind on project submissions. Payment also overdue.',
    createdAt: 'Mar 12, 2024'
  }],

  flagReason: 'Behind on project submissions'
},
{
  id: 'S-1011',
  name: 'Emily Blunt',
  email: 'emily.b@movies.com',
  cohortId: 'C-2024-002',
  cohortName: 'Winter 2024 Product Strategy',
  status: 'Completed',
  mentor: 'Mike Ross',
...
export function StudentsPage({
  ...
  const [cohortFilter, setCohortFilter] = useState<string | null>(
    initialCohortId || null
  );
  // Update cohort filter when prop changes
  useEffect(() => {
    if (initialCohortId) {
      setCohortFilter(initialCohortId);
    }
  }, [initialCohortId]);
  // Pagination State
  ...
  // Handle initial student selection from search
  useEffect(() => {
    if (initialStudentId) {
